...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Tab' || e.key === 'CapsLock' || e.key === 'Shift' || e.key === 'Control' || e.key === 'Meta' || e.key === 'Alt' || e.key === 'Backspace' || e.key === 'Enter' || e.key === 'ArrowUp' || e.key === 'ArrowDown' || e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
        textArea.textContent += '';
    } else {
